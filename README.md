Modified version with SASS support. Put your sass files like abc.scss (or _abc.scss which should be imported by other sass files) into the scss directory.
For each .scss files which doesn't start with `_` a .css file will be generated in src directory. You can then import those css files into your js files as usual.
